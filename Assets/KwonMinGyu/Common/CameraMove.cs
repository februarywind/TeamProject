...
public class CameraMove : MonoBehaviour
{
    ...
    // 카메라가 회전할 수 있는 최대 각도 (상하 제한)
    [SerializeField] private float _rotationUpLimit = 50f;  

    // 카메라가 회전할 수 있는 최소 각도 (상하 제한)
    [SerializeField] private float _rotationDownLimit = -20f;  
    ...
    public CameraPos CameraPosition()
    {
        ...
        // 카메라의 x좌표값이 대략 +-(_offset.z - 2) 일때 방향 전환
        ...
        else if (transform.position.z - _cubeTransform.position.z > 0) // 카메라의 위치가 양 옆이 아니고 z좌표값이 양수라면 카메라는 큐브의 윗 방향에 있음
